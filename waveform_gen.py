...
def waveform_gen_sine(bar, tempo, sample_rate, piece_list,beat_list):
    ...
    waveform = [0] * (bar + 1) * 8 * sample_rate
...
def waveform_gen_exp(bar, tempo, sample_rate, piece_list,beat_list, waveform, note_duration):
    # volume and duration for note
    volume = 30
    ...
    for note in piece_list:
        ...
        for n in range(note_sample):
            for i in note:
                waveform[t+n] = waveform[t+n] + volume * np.sin(2 * 3.142 * note_freq[i][1] * n / sample_rate)*np.exp(-0.00005*n) * 127 + 128
            #waveform[t+n] = 127 * waveform[t+n] + 128
        count = count + 1
        t = t + int(sample_rate * beat_list[count])
...
#################################################################################################################################
 # exponential envelope, sounds more natural, exponential decay
def waveform_gen_exp_short(bar, tempo, sample_rate, piece_list,beat_list, waveform, note_duration):
    # volume and duration for note
    volume = 30
    ...
    for note in piece_list:
        ...
        for n in range(note_sample):
            for i in note:
                waveform[t+n] = waveform[t+n] + volume * np.sin(2 * 3.142 * note_freq[i][1] * n / sample_rate)*np.exp(-0.0005*n) * 127 + 128
            #waveform[t+n] = 127 * waveform[t+n] + 128
        count = count + 1
        t = t + int(sample_rate * beat_list[count])
...
################################################################################################
 # exponential envelope, sounds more natural, exponential decay
def waveform_gen_new(bar, tempo, sample_rate, piece_list, waveform, note_duration):
    # volume and duration for note
    volume = 15
    ...
    for note in piece_list:
        # generate waveform using note in piece list
        if note == 0:
            t = t + int(sample_rate * tempo)

        else: 
            for n in range(note_sample):
                for i in note:
                    waveform[t+n] = waveform[t+n] + volume * np.sin(2 * 3.142 * note_freq[i][1] * n / sample_rate)*np.exp(-0.00000008*n*n) * 127 + 128
            t = t + int(sample_rate * tempo)
        count = count + 1
...
################################################################################################
 # exponential envelope, sounds more natural, exponential decay
def waveform_gen_new_2(bar, tempo, sample_rate, piece_list, waveform, note_duration):
    # volume and duration for note
    volume = 20
